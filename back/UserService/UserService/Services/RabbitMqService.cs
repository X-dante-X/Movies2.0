...
public static class RabbitMqService
{
    ...
    public static async Task<List<UserResponse>> GetUserNameById(List<string> userIds)
    {
        ...
        await channel.BasicPublishAsync(exchange: "", routingKey: "user_request", mandatory: true, basicProperties: props, body: messageBytes);
        Console.WriteLine("Message sent to RabbitMQ.");

        var responseJson = await tcs.Task;
        Console.WriteLine("Response received.");

        var userNames = JsonSerializer.Deserialize<List<UserResponse>>(responseJson);
        if (userNames == null)
        {
            throw new Exception("Not found");
        }
